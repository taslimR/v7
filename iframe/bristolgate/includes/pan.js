(function() {
	...
	var width=790,height=600,padding=0;
	
	var rp=d3.select("#riskpanel").html();
	var gicss=["global","staples","energy","financials","healthcare","industrials","it","materials","realestate","telecom","utilities"];
	var gicsHash={"Consumer_Discretionary":"global","Consumer_Staples":"staples","Energy":"energy","Financials":"financials","Health_Care":"healthcare","Industrials":"industrials","Information_Technology":"it","Materials":"materials","Real_Estate":"realestate","Telecommunication_Services":"telecom","Utilities":"utilities"}
	var interface={
		buttons:{
		applications:[
		...
		leftPan:{
			applications:["Bandwidth:","% of total marketcap:","HD movie equivalent:","Sessions consumed:","# of apps:"],
	...
	var technologies=["Low","Medium","High"];
	var catbysubcat={};
	var subcats=[];
	...
	var catxy={"Low":[230,186],"Medium":[460,186],"High":[690,186]};
	...
	var techxy={"Low":[230,186],"Medium":[460,186],"High":[690,186]};
	...
	pan.filter = function(criteria) {
		...
		if (pan.main=="applications") {
			...
			//pan.selection=pan.selection.sort(function(a,b) {return b.marketcap-a.marketcap}).slice(0,100);

		

			var selectionB=d3.sum(pan.selection,function(d) {return +d.marketcap;})
...
//				pan.selection=pan.selection.filter(function(d) {return top25.indexOf(d.application)>-1&&d.category!="total";})	
		...
		pan.dataLeft[0]=d3.select("#gics_sector").property("value")
	...
	pan.computePos=function() {
		...
		largeenough.sort(function(a,b) {return b.marketcap-a.marketcap}).slice(0,100);


		var c1=largeenough.map(function(d) {return srsly(d.marketcap);})
		...
		var catsum=d3.nest()	
			...
			.rollup(function(d) {return d3.sum(d,function(e) {return srsly(e.marketcap);})})
			.map(pan.selection);
		...
		categories.forEach(function(cat) {
			...
			var scat_val=scat.map(function(d) {return srsly(d.marketcap);})
...
//			var spro_val=spro.map(function(d) {return srsly(d.marketcap);})
...
//		})
		...
		technologies.forEach(function(tech) {
			...
			var stech_val=stech.map(function(d) {return srsly(d.marketcap);})
		...
		pan.selection.forEach(function(d) {
			d.pos[3]=({x:xScale(d.frequency), r:rScale(srsly(d.marketcap)), y:yScale[d.category]})
		})
		} else {
	...
	pan.gendata= function() {
		pan.data=d3.range(500).map(function(i) 
			{var d={id:i,
				...
				marketcap: (10+p_rand(900))*(10+p_rand(900)),
				risk:p_rand(5)+1
			};
			return d;
		})
	...
	pan.readdata=function() {
		d3.csv(pan.appfile,function(error,csv) {
			...
			pan.totalB=d3.sum(pan.data,function(d) {
				return (d.gics==="global"?+d.marketcap:0)
			})
			pan.totalS=d3.sum(pan.data,function(d) {
				return (d.gics==="global"?+d.sessions:0)
			})
			subcats.forEach(function(s) {
				gicss.forEach(function(r) {
					id=id+1;
					var selection=pan.data.filter(function(d) {return (d.subcategory==s)&&(d.gics==r)});
					subcat={
						name:s,
						type:"subcategory",
						...
						id:id,
						marketcap:d3.sum(selection,function(d) {return +d.marketcap;}),
			...
			var s0=pan.data;
			gicss.forEach(function(r) {
				var s1=pan.data.filter(function(d) {return d.gics==r});
				categories.concat("total").forEach(function(c) {
					...
					technologies.concat("total").forEach(function(t) {
						...
						pan.smallApps.push({
							...
							technology:t,
							gics:r,
							marketcap:d3.sum(small,function(d) {return +d.marketcap;}),
		...
		d3.csv(pan.threatfile,function(error,csv) {
			...
			csv.forEach(function(d,index) {
				...
				if (key in globalhash) {
					...
					i=i+1;
					globalhash[key]=i;
					pan.threats[i]={name:d.name,type:"threat",detections:+d.detections,attack:d.attack,application:d.application,category:d.category,threatType:d.threatType,severity:d.severity,severityVal:d.severityVal,risk:"threat",gics:"global",id:i};
				}
				...
				var selection0=pan.threats.filter(function(d) {return d.application==a});
					["exploit","malware","total"].forEach(function(t) {
						...
						threatCats.concat("total").forEach(function(tc) {
							...
							selection2=selection1.filter(function(d) {return d.gics==r}); 
	...
	pan.drawview = function(m) {
		...
		if(mode==0) {
			...
			psb.append("text").text(pan.main=="applications"?"total marketcap occupied":"number of detections").attr({x:800,y:322,"text-anchor":"middle"})
		...
		if (mode==3) {
			if(pan.main=="applications") {
			psb.append("text").text("Frequency: Forecast dividend growth")
			...
			psb.selectAll(".labelX").data(d3.range(6)).enter().append("text")
				.text(function(d) {/*return d3.format("2%")(d*.2);*/})//%label by hasnain
		...
		if(mode==1) {
				pan.svg.selectAll(".labels").data(pan.main=="applications"?categories:threatCats).enter().append("text")
					...
					.attr({x:function(d)   {return pan.main=="applications"?(catxy[d][0]):(threatCatxy[d][0]);},
						   y:function(d,i) {return i<({"applications":3,"threats":4}[pan.main])?20:300;},
			   "text-anchor":"middle"})
			}
			if(mode==2) {
				pan.svg.selectAll(".labels").data(pan.main=="applications"?technologies:threatTypes).enter().append("text")
					...
					.attr({x:function(d)   {return pan.main=="applications"?(techxy[d][0]):(threatxy[d][0])},
						   y:function(d,i) {return i<3?20:350;},
			   "text-anchor":"middle"})
			}
		...
		circles.on("mouseover",function(d) {
			...
			if(pan.main=="applications") {
				if(d3.select("#select_dataset").property("value").toLowerCase()=="applications by subcategory") {
					...
					{label:"Bandwidth:",value:d3.format(",")(+d.marketcap)},
					{label:"HD movies equivalent:",value:d3.format(",")(+d.hd)},
					{label:"% of total marketcap:",value:d3.format("4.2%")(+d.marketcap/pan.totalB)},	
				...
				infotip=[
					...
					{label:"Frequency:",value:d3.format(",")(+d.frequency)},
...
pan.setInterface = function() {
		...
		if(!d3.select("#gics_sector").property("value")) {d3.select("#gics_sector").select("option").property("selected",true);}
		...
		d3.selectAll("#cat-app,#eps,#dqec-app").on("click",function() {
		...
		d3.selectAll("#last").on("click",function() {
			//if(!d3.select(this).classed("disabled")) {
				//d3.select("#sidebar").selectAll("a").classed("active",0)
				d3.select("#view_selection").selectAll("a").classed("active",0);
				d3.select(this).classed("active",1);
	...
	pan.view = function(options) {
		...
		d3.select("#gics_sector").select("option").property("selected",true)
		pan.setInterface();

		

		d3.select("#gics_sector").property("value","Consumer_Discretionary").on("change",function() {
			d3.select("#sidebar").selectAll("a").classed("active",0)
			var v=d3.select(this).property("selectedIndex");
			pan.criteria["gics"]=["global","staples","energy","financials","healthcare","industrials","it","materials","realestate","telecom","utilities"][v];
			pan.filter(pan.criteria);
			pan.drawview(pan.mode);
		})
		...
		d3.select("#sidebar").selectAll("a").on("click",function(d,i) {
			...
			if(i==0) {
				...
				d3.select("#gics_sector").property("value","Worldwide");
				d3.select("#all-app").classed("active",1);
				d3.selectAll("#tec-app,#fre-app").classed("disabled",1);
				pan.mode=0;
				pan.risk="All";
			...
			if(i==7) {
				...
				pan.risk="All";
				pan.dataLeft[2]="All";
				pan.criteria={gics:"global",subcategory:"proxy-tunnels"};
				pan.filter(pan.criteria);
				pan.setInterface();
				pan.drawview(pan.mode);
			}
